...
function toggleMenu() {
  ...
  if (icon) {
    icon.textContent = menu.style.display === "block" ? "✖" : "☰";
  }
}
...
function nextStory() {
  storySlider.scrollBy({ left: 320, behavior: "smooth" });
}
...
function nextReview() {
  reviewSlider.scrollBy({ left: 320, behavior: "smooth" });
}
...
setInterval(() => {
  ...
  if (
    reviewSlider.scrollLeft + reviewSlider.clientWidth >=
